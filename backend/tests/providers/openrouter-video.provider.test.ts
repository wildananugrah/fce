...
// upload() returns the public URL; we don't need delete() for these tests.
function makeMockMinio(publicUrl: string) {
	return {
...
describe("OpenRouterVideoAnalyzerProvider", () => {
	describe("analyzeVideo", () => {
		it("uploads bytes to MinIO, sends public URL as video_url to OpenRouter, returns parsed analysis", async () => {
			const minio = makeMockMinio("https://minio.example.com/test-bucket/video-123.mp4");
			...
			// The request body contained a video_url pointing at the MinIO public URL.
