...
export type TrashItemType = "brand" | "product" | "topic" | "content";
...
/**
 * Aggregates archived items across the four supported entity types and
 * annotates each with its hard-delete `expiresAt` (archivedAt + TTL). The
 * trash view in Workspace Settings consumes this.
 *
 * We intentionally collapse descendants under an archived ancestor: when a
 * brand is archived, its products / topics / content are hidden from the
 * main lists (via join filters) but we don't show each of them as a
 * separate trash row. Restoring the brand makes everything reappear as if
 * nothing happened, which matches the user's "move to trash / restore"
 * mental model and keeps the trash UI from becoming overwhelming.
 */
export class TrashService {
	...
	async list(workspaceId: string): Promise<TrashItem[]> {
		const [brands, products, topics, outputs] = await Promise.all([
		...
		// Most recently archived first — people usually want to restore what
		// they just trashed.
